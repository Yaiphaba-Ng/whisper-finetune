...
class Fleurs(torch.utils.data.Dataset):
    ...
    Downloads to ./dataset if not already present, and loads from there.
    """
    def __init__(self, lang, split="test", subsample_rate=1, device=DEVICE, max_samples=None):
        os.makedirs("./dataset", exist_ok=True)
        tar_path = os.path.join("./dataset", f"{lang}.tgz")
...
# %%
dataset = Fleurs(lang, max_samples=10)  # load exact no. of samples
...
# ask user for test no.:
next_number = input("Enter the test number: ")

# Create a subfoler for each lang
lang_folder = os.path.join(output_dir, lang)
...
csv_filename = os.path.join(lang_folder, f"({lang})_test_{next_number}-{datetime}.csv")
json_filename = os.path.join(lang_folder, f"({lang})_test_{next_number}-{datetime}.json")
md_filename = os.path.join(lang_folder, f"({lang})_test_{next_number}-{datetime}.md")

# Save the DataFrame to CSV
data.to_csv(csv_filename, index=False)

# Save the DataFrame to JSON
# Convert DataFrame to dictionary and then save as JSON
data_dict = data.to_dict(orient='records')
with open(json_filename, 'w', encoding='utf-8') as f:
    json.dump(data_dict, f, ensure_ascii=False, indent=4)

# Save the DataFrame to Markdown
# Convert DataFrame to Markdown string
markdown_string = data.to_markdown(index=False)
...
zip_filename = f"{output_dir}_{datetime}"
...
zip_filename = f"{zip_dir}/{datetime}"
shutil.make_archive(zip_filename, 'zip', output_dir)
...
for audio, text in dataset:
    ...
    print(f"Transcription: {data['transcription'].iloc[0]}")
    display(Audio(audio.numpy(), rate=whisper.audio.SAMPLE_RATE))

# %%
from jiwer import wer, cer

# Calculate WER and CER for the whole dataset
wer_score = wer(data['reference'].tolist(), data['transcription'].tolist())
cer_score = cer(data['reference'].tolist(), data['transcription'].tolist())
